...
(:
 ...
 :
 : Version:     $Id: fgases-2019.xquery 22084 2019-03-10 13:31:33Z katsanas $
...
import module namespace cutil = "http://converters.eionet.europa.eu/fgases/cutil" at "fgases-common-util-2019.xquery";
(: UI utility methods for build HTML formatted QA result:)
import module namespace uiutil = "http://converters.eionet.europa.eu/fgases/ui" at "fgases-ui-util-2019.xquery";
import module namespace fgases = 'http://converters.eionet.europa.eu/fgases/helper' at "fgases-helper-2019.xquery";
    ...
    return
        if ($err_flag) then
            ...
            let $actualTransactionYears :=
                for $actualYear in 2019 to fn:year-from-date(fn:current-date()) - 1
...
as element(div)* {
    ...
    return
        ...
        let $tr_3A := cutil:numberIfEmpty($gas/tr_03A/totalAmountForRow, 0)
...
declare function xmlconv:qc24101($doc) as element(div)*
{
(: QC 24101 :)
    let $err_text := "The amount placed on the market in the United Kingdom can not exceed the total amount placed on the market. Please revise your data."
    return
        if (not(fgases:is-P-HFC($doc)) and not(fgases:is-I-HFC($doc))) then
            ()
        else
            let $err_flag :=
            for $gas in $doc/ReportedGases
                let $gasAmount13aA := cutil:getZeroIfNotNumber($doc/F2_S5_exempted_HFCs/Gas[GasCode=$gas/GasId]/tr_13aA/Amount)
                let $gasAmount4D := cutil:getZeroIfNotNumber($doc/F1_S1_4_ProdImpExp/Gas[GasCode=$gas/GasId]/tr_04D/Amount)
                let $gasAmount4I := cutil:getZeroIfNotNumber($doc/F1_S1_4_ProdImpExp/Gas[GasCode=$gas/GasId]/tr_04I/Amount)
                
                let $gasAmount13aB := cutil:getZeroIfNotNumber($doc/F2_S5_exempted_HFCs/Gas[GasCode=$gas/GasId]/tr_13aB/Amount)
            where (($gasAmount13aA >= 0) and ($gasAmount13aB > (($gasAmount13aA+$gasAmount4D)-$gasAmount4I)))
            return data($gas/Name)
            return uiutil:buildRuleResult("24101", "13aA", $err_text, $xmlconv:BLOCKER,count($err_flag)>0, $err_flag, "Invalid gases are: ")

};

declare function xmlconv:qc24102($doc) as element(div)*
{
(: QC 24102 :)
    let $err_text := "The amount placed on the market in the United Kingdom can not exceed the total amount placed on the market. Please revise your data."
    return
        if (not(fgases:is-P-HFC($doc)) and not(fgases:is-I-HFC($doc))) then
            ...
            where (($gasAmount13aA < 0) and ($gasAmount13aB <(($gasAmount13aA+$gasAmount4D)-$gasAmount4I)))
            return data($gas/Name)
            return uiutil:buildRuleResult("24102", "13aA", $err_text, $xmlconv:BLOCKER,count($err_flag)>0, $err_flag, "Invalid gases are: ")

};

declare function xmlconv:qc24103($doc) as element(div)*
{
(: QC 24103 :)
    let $warning_text := "Your company is UK-based and you stated that you have not placed any gases on the UK market. Please make sure the data you entered is correct."
    return
        if (not(fgases:is-P-HFC($doc)) and not(fgases:is-I-HFC($doc))) then
            ()
        else
            let $warning_flag :=
                for $gas in $doc/ReportedGases
                    ...
                    let $gasAmount13aB := cutil:getZeroIfNotNumber($doc/F2_S5_exempted_HFCs/Gas[GasCode=$gas/GasId]/tr_13aB/Amount)
                where (fgases:contains-HFC($gas) and ($gasAmount13aA != 0) and ($gasAmount13aB = 0) and ($doc/GeneralReportData/Company/Country/Code/text()="UK" or $doc/GeneralReportData/Company/EuLegalRepresentativeCompany/Country/Code/text()="UK"))
                return data($gas/Name)
            return uiutil:buildRuleResult("24103", "13aA,13aB", $warning_text, $xmlconv:WARNING, count($warning_flag)>0, $warning_flag, "Invalid gases are: ")
...
(: apply to rule 2098 :)
    ...
    let $err_flag :=
        for $gas in $doc/ReportedGases
        let $sum := cutil:getZeroIfNotNumber($doc/F1_S1_4_ProdImpExp/Gas[GasCode=$gas/GasId]/tr_02A/totalAmountForRow) + cutil:getZeroIfNotNumber($doc/F1_S1_4_ProdImpExp/Gas[GasCode=$gas/GasId]/tr_04C/Amount)
...
as element(div)*
{
    let $errorText := "Destroyed own production (1D) must not exceed the sum of your total production and 1st January stocks from own import/production not placed on the market (1Ab + 4C). Please revise your data."
    let $tr01DGases := fgases:get-gas-amounts($report/F1_S1_4_ProdImpExp, "tr_01D" )
    let $tr01AbGases := fgases:get-gas-amounts($report/F1_S1_4_ProdImpExp, "tr_01Ab" )
    let $tr04CGases := fgases:get-gas-amounts($report/F1_S1_4_ProdImpExp, "tr_04C" )
    for $gas in $tr01AbGases
    let $gasCode := $gas/gasId
    let $tr01DAmount := cutil:numberIfEmpty($gas/amount, 0)
    let $tr01AbAmount := cutil:numberIfEmpty($tr01AbGases[gasId=$gasCode]/amount, 0)
    let $tr04CAmount := cutil:numberIfEmpty($tr04CGases[gasId=$gasCode]/amount, 0)
    where
        $tr01DAmount > sum(($tr01AbAmount, $tr04CAmount))
    return
    ...
    return
        ...
        return
            if($condition and not(xmlconv:Section9DocumentIsVerified($doc))) then
                uiutil:buildRuleResult("2044", "09C", $errorText, $xmlconv:BLOCKER, true(), (), "")
            else
                ()
};
...
as element(div)*
{
    ...
    let $totalErrors :=
        ...
        let $errors :=
            for $country in $countrySpecificData
            let $countryName := cutil:getTransactionCountryName($countryDefinition, $country/Country/CountryId)
            let $errorText := xmlconv:getQC2409ErrorMessageCountry($countryName, $gasName, $sectionName)
            where not(cutil:transactionHasMandatoryComment($country/Country))
            return uiutil:buildRuleResult("2409", $transactionCode, $errorText, $xmlconv:BLOCKER, true(), (), "")
        return $errors
    return $totalErrors
};
...
as element(div)*
{
  (:  let $invalidGasTransactions :=
        for $gas in $gases        
        let $transactionData := $gas/tr_01A_fs/CountrySpecific/Country
        where not(cutil:transactionHasMandatoryComment($transactionData))
        return
            let $gasName := cutil:getGasNameByGasCode($reportedData, $gas/GasCode)
...
as element(div)*
{
    ...
    let $totalErrors :=
        ...
        let $gasCode := $gas/GasCode
        let $gasName := cutil:getGasNameByGasCode($reportedData, $gasCode)
        let $countrySpecificData := $gas/*[local-name()=$atransaction]/CountrySpecific
        let $errors :=
            if ( not(not($countrySpecificData/Country/text()[normalize-space(.) != '']) and $countrySpecificData/Country/*)) then
                ()
            else
       ...
       $gases as element(Gas)*) as element(div)*
{
    ...
    let $qcCode := "24106"
    let $result := xmlconv:qc24100($reportedData, $countryDefinition, $gases, "02A_pp", "tr_02A", $err, $qcCode)
    return $result
};
       ...
       $gases as element(Gas)*) as element(div)*
{
    ...
    let $qcCode := "24106"
    let $result := xmlconv:qc24100($reportedData, $countryDefinition, $gases, "03A_pp", "tr_03A", $err, $qcCode)
    return $result
};
...
{        
    if(not(fgases:is-Eq-I-RACHP-HFC($report))) then
        ...
        return                
            if($val13A < $val13D) then 
                if ($val13A < 0 and $val13D = 0)  then
                    ()
                else
...
as element(div)
{
    ...
    let $resultDiv :=
        ...
        then
            ...
            let $r24101 := xmlconv:qc24101($doc)
            let $r24102 := xmlconv:qc24102($doc)
            let $r24103 := xmlconv:qc24103($doc)
            ...
            let $r2301_11A1 := xmlconv:validateTransactionAmountRange($doc, "11A01", 0.1, 10.0, "kg/piece", "2300")
            let $r2301_11A2 := xmlconv:validateTransactionAmountRange($doc, "11A02", 0.5, 50.0, "kg/piece", "2300")
            let $r2301_11A3 := xmlconv:validateTransactionAmountRange($doc, "11A03", 0.1, 50.0, "kg/piece", "2300")
            let $r2301_11A4 := xmlconv:validateTransactionAmountRange($doc, "11A04", 1, 50.0, "kg/piece", "2300")
            let $r2301_11A5 := xmlconv:validateTransactionAmountRange($doc, "11A05", 0.1, 3.0, "kg/piece", "2300")
            let $r2301_11A6 := xmlconv:validateTransactionAmountRange($doc, "11A06", 1, 50.0, "kg/piece", "2300")
            let $r2301_11A7 := xmlconv:validateTransactionAmountRange($doc, "11A07", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11A8 := xmlconv:validateTransactionAmountRange($doc, "11A08", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11A9 := xmlconv:validateTransactionAmountRange($doc, "11A09", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11A10 := xmlconv:validateTransactionAmountRange($doc, "11A10", 0.5, 10.0, "kg/piece", "2300")
            let $r2301_11A11 := xmlconv:validateTransactionAmountRange($doc, "11A11", 0.1, 50.0, "kg/piece", "2300")
            let $r2301_11A12 := xmlconv:validateTransactionAmountRange($doc, "11A12", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11A13 := xmlconv:validateTransactionAmountRange($doc, "11A13", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11A14 := xmlconv:validateTransactionAmountRange($doc, "11A14", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11B1 := xmlconv:validateTransactionAmountRange($doc, "11B01", 0.04, 2.0, "kg/piece", "2300")
            let $r2301_11B2 := xmlconv:validateTransactionAmountRange($doc, "11B02", 0.03, 10.0, "kg/piece", "2300")
            let $r2301_11B3 := xmlconv:validateTransactionAmountRange($doc, "11B03", 0.04, 10.0, "kg/piece", "2300")
            let $r2301_11B4 := xmlconv:validateTransactionAmountRange($doc, "11B04", 0.5, 50.0, "kg/piece", "2300")
            let $r2301_11B5 := xmlconv:validateTransactionAmountRange($doc, "11B05", 1, 50.0, "kg/piece", "2300")
            let $r2301_11B6 := xmlconv:validateTransactionAmountRange($doc, "11B06", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11B7 := xmlconv:validateTransactionAmountRange($doc, "11B07", 1, 50.0, "kg/piece", "2300")
            let $r2301_11B8 := xmlconv:validateTransactionAmountRange($doc, "11B08", 1, 50.0, "kg/piece", "2300")
            let $r2301_11B9 := xmlconv:validateTransactionAmountRange($doc, "11B09", 1, 50.0, "kg/piece", "2300")
            let $r2301_11B10 := xmlconv:validateTransactionAmountRange($doc, "11B10", 1, 50.0, "kg/piece", "2300")
            let $r2301_11B11 := xmlconv:validateTransactionAmountRange($doc, "11B11", 1, 50.0, "kg/piece", "2300")
            let $r2301_11B12 := xmlconv:validateTransactionAmountRange($doc, "11B12", 0.1, 50.0, "kg/piece", "2300")
            let $r2301_11B13 := xmlconv:validateTransactionAmountRange($doc, "11B13", 0.5, 50.0, "kg/piece", "2300")
            let $r2301_11B14 := xmlconv:validateTransactionAmountRange($doc, "11B14", 1, 50.0, "kg/piece", "2300")
            let $r2301_11C := xmlconv:validateTransactionAmountRange($doc, "11C", 0.15, 0.5, "kg/piece", "2300")
            let $r2301_11D1 := xmlconv:validateTransactionAmountRange($doc, "11D01", 0.05, 50.0, "kg/piece", "2300")
            let $r2301_11D2 := xmlconv:validateTransactionAmountRange($doc, "11D02", 0.05, 50.0, "kg/piece", "2300")
            let $r2301_11D3 := xmlconv:validateTransactionAmountRange($doc, "11D03", 0.1, 50.0, "kg/piece", "2300")
            let $r2301_11E1 := xmlconv:validateTransactionAmountRange($doc, "11E01", 0.7, 13.0, "kg/piece", "2300")
            let $r2301_11E2 := xmlconv:validateTransactionAmountRange($doc, "11E02", 0.6, 1.6, "kg/piece", "2300")
            let $r2301_11E3 := xmlconv:validateTransactionAmountRange($doc, "11E03", 10.0, 50.0, "kg/piece", "2300")
            let $r2301_11E4 := xmlconv:validateTransactionAmountRange($doc, "11E04", 0.035, 5.0, "kg/piece", "2300")
            let $r2301_11F1 := xmlconv:validateTransactionAmountRange($doc, "11F01", 0.3, 1.5, "kg/piece", "2300")
            let $r2301_11F2 := xmlconv:validateTransactionAmountRange($doc, "11F02", 1.0, 20.0, "kg/piece", "2300")
            let $r2301_11F3 := xmlconv:validateTransactionAmountRange($doc, "11F03", 0.3, 1.5, "kg/piece", "2300")
            let $r2301_11F4 := xmlconv:validateTransactionAmountRange($doc, "11F04", 0.7, 1.5, "kg/piece", "2300")
            let $r2301_11F5 := xmlconv:validateTransactionAmountRange($doc, "11F05", 0.5, 2.5, "kg/piece", "2300")
            let $r2301_11F6 := xmlconv:validateTransactionAmountRange($doc, "11F06", 1.0, 35.0, "kg/piece", "2300")
            let $r2301_11F7 := xmlconv:validateTransactionAmountRange($doc, "11F07", 0.3, 50.0, "kg/piece", "2300")
            let $r2301_11F8 := xmlconv:validateTransactionAmountRange($doc, "11F08", 2.0, 10.0, "kg/piece", "2300")
            let $r2301_11F9 := xmlconv:validateTransactionAmountRange($doc, "11F09", 0.1, 50.0, "kg/piece", "2300")
            let $r2301_11H1 := xmlconv:validateTransactionAmountRange($doc, "11H01", 10.0, 50.0, "kg/t", "2300")
            ...
            let $r2301_11H4 :=
                if($doc/F7_s11EquImportTable/TR_11H4_Unit="metrictonnes") then
                    xmlconv:validateTransactionAmountRange($doc, "11H04", 10.0, 50.0, "kg/t", "2300")
                else if($doc/F7_s11EquImportTable/TR_11H4_Unit="cubicmetres") then
                    xmlconv:validateTransactionAmountRange($doc, "11H04", 1.0, 10.0, "kg/m3", "2300")
                else if($doc/F7_s11EquImportTable/TR_11H4_Unit="pieces") then
                        xmlconv:validateTransactionAmountRange($doc, "11H04", 1.0, 10.0, "kg/pieces", "2300")
                    else
                        ()
            let $r2301_11I := xmlconv:validateTransactionAmountRange($doc, "11I", 3.0, 50.0, "kg/piece", "2300")
            ...
            let $r2301_11L := xmlconv:validateTransactionAmountRange($doc, "11L", 0.01, 50.0, "kg/piece", "2300")
            let $r2301_11M := xmlconv:validateTransactionAmountRange($doc, "11M", 1.0, 50.0, "kg/piece", "2300")
            let $r2301_11N := xmlconv:validateTransactionAmountRange($doc, "11N", 0.5, 50.0, "kg/piece", "2300")
            let $r2301_11O := xmlconv:validateTransactionAmountRange($doc, "11O", 0.2, 50.0, "kg/piece", "2300")
            let $r2301_11P :=
                if($doc/F7_s11EquImportTable/TR_11P_Unit="metrictonnes") then
                    xmlconv:validateTransactionAmountRange($doc, "11P", 0.01, 50.0, "kg/t", "2300")
                else if($doc/F7_s11EquImportTable/TR_11P_Unit="cubicmetres") then
                    xmlconv:validateTransactionAmountRange($doc, "11P", 0.01, 50.0, "kg/m3", "2300")
                else if($doc/F7_s11EquImportTable/TR_11P_Unit="pieces") then
                        xmlconv:validateTransactionAmountRange($doc, "11P", 0.01, 50.0, "kg/pieces", "2300")
                    else
                        ()
            ...
            let $r24109 := xmlconv:qc24109($doc,$doc/F1_S1_4_ProdImpExp/tr_03A_Countries,$doc/F1_S1_4_ProdImpExp/Gas)
