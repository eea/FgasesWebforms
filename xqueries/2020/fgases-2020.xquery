...
import module namespace cutil = "http://converters.eionet.europa.eu/fgases/cutil" at "fgases-common-util-2020.xquery";
(: UI utility methods for build HTML formatted QA result:)
import module namespace uiutil = "http://converters.eionet.europa.eu/fgases/ui" at "fgases-ui-util-2020.xquery";
import module namespace fgases = 'http://converters.eionet.europa.eu/fgases/helper' at "fgases-helper-2020.xquery";
    ...
    return
        if ($err_flag) then
            ...
            let $actualTransactionYears :=
                for $actualYear in 2020 to fn:year-from-date(fn:current-date()) - 1
...
as element(div)*
{
    let $errorText := "It appears implausible that your import for own destruction reported in [5A, 1A_a_own or 1B] exceeds total destruction reported in section 8. Please revise your data or add an explanation in section [5A, 1A_a_own or 1B]."
    let $isD := $report/GeneralReportData/Activities/D = 'true'
    return
        if (not($isD)) then
            ()
            ...
            return
                ...
                return
                    ...
                    let $gasAmount1BComment := $report/F1_S1_4_ProdImpExp/Gas[GasCode = $gasCode]/tr_01B
                    let $gasAmount1A_a_ownComment := $report/F1_S1_4_ProdImpExp/Gas[GasCode = $gasCode]/tr_01A_a_own
                    let $errorType :=
                        if((string-length($ownTr05AReported/Comment) > 0) or (string-length($gasAmount1BComment/Comment) > 0) or (string-length($gasAmount1A_a_ownComment/Comment) > 0)) then
                            $xmlconv:WARNING
                        else
                            $xmlconv:BLOCKER
                    return
                        if (($ownTr05AreportedAmount + $tr01A_a_ownAmount + $tr01BAmount) > $tr08DAmount) then
...
(: QC 24101 :)
    ...
    return
        if (not(fgases:is-P-HFC($doc)) and not(fgases:is-I-HFC($doc))) then
            ...
            where (($gasAmount13aA >= 0) and ((-$gasAmount4D > $gasAmount13aB) or ($gasAmount13aB > ($gasAmount13aA + $gasAmount4D))))
...
as element(div)
{
    ...
    let $resultDiv :=
        ...
        then
            ...
            let $r2301_11A7 := xmlconv:validateTransactionAmountRange($doc, "11A07", 0.1, 10.0, "kg/piece", "2300")
            ...
            let $r2301_11H4 :=
                if($doc/F7_s11EquImportTable/TR_11H4_Unit="metrictonnes") then
                    ...
                    xmlconv:validateTransactionAmountRange($doc, "11H04", 1.0, 10.0, "kg/m3", "2300")
                        ...
                        xmlconv:validateTransactionAmountRange($doc, "11H04", 1.0, 10.0, "kg/piece", "2300")
                    else
                        ()
