...
(:
 ...
 :
 : Version:     $Id: fgases-2025.xquery 22084 2024-03-10 13:31:33Z katsanas $
 : Created:     20 November 2014
 : Copyright:   European Environment Agency
 :)
(:~
 : Reporting Obligation: http://rod.eionet.europa.eu/obligations/713
 : XML Schema: http://dd.eionet.europa.eu/schemas/fgases-2025/FGasesReporting.xsd
 :
...
import module namespace cutil = "http://converters.eionet.europa.eu/fgases/cutil" at "fgases-common-util-2025.xquery";
(: UI utility methods for build HTML formatted QA result:)
import module namespace uiutil = "http://converters.eionet.europa.eu/fgases/ui" at "fgases-ui-util-2025.xquery";
import module namespace fgases = 'http://converters.eionet.europa.eu/fgases/helper' at "fgases-helper-2025.xquery";
...
(: check webform status, it has to be completed - click "Close report" button on Finish tab :)
    ...
    let $status := lower-case(data($doc/GeneralReportData/@status))
    let $err_flag := $status != "completed" and $status != "copied"
...
declare function xmlconv:rule_CopyStatus($doc as element())
as element(div)? {
    let $err_text := "This report is based on a submission from the previous transaction year which was not updated. It is not possible to submit a previous report without opening the report and checking the data. Please create a new envelope, load the previous report, open the questionnaire and check the data. You can then finalize your report and submit it to DG Clima/EEA. Thank you!"
    let $err_flag := lower-case(data($doc/GeneralReportData/@status)) = "copied"
    return
        if ($err_flag) then
            uiutil:buildRuleResult("status", "", $err_text, $xmlconv:BLOCKER, $err_flag, (), "")
        else
    ...
    return
        if(not(fgases:is-P-HFC($report))) then
            ...
            let $gasAmounts :=
                ...
                let $amount1A := cutil:numberIfEmpty($gas/tr_01A/Amount, 0)
                let $amount1D := cutil:numberIfEmpty($gas/tr_01D/Amount, 0)
                where
                    not(fgases:is-blend($reportedGas))
                    and not(fgases:is-unspecified-mix($reportedGas))
                    and fgases:contains-HFC($reportedGas)
                    and ($amount1A > 0 or $amount1D > 0)
                return $gas
            return
...
as element(div)*
{
    ...
    let $invalidGases :=
        ...
        let $d := number($gas/tr_08D/Amount)
        let $e := number($gas/tr_08E/Amount)
        let $f := number($gas/tr_08F/Amount)
        where $authIsSelected and not($d > 0 or $e > 0 or $f > 0)
        return $gas/GasId
...
as element(div)* {
  ...
  let $errorText :=
    'You selected "Importer of other products or equipment" in the activity selection. Please report on section 11H-11P or on non-HFCs or mixtures not containing HFCs in section 11A-11F or unselect the activity.'
      ...
      let $hasPositiveAmount :=
        exists(
          $gases/(
            ...
            tr_11P/Amount |
            tr_11Q/Amount
          )[normalize-space(.) != '' and xs:double(.) > 0]
        )

      return
        if (empty($gases) or not($hasPositiveAmount)) then
...
declare function xmlconv:qc2100($report as element())
    ...
    let $xmlCompanyId := xs:integer($report/GeneralReportData/Company/CompanyId)
        ...
...
declare function xmlconv:qc2107($report as element())
as element(div)*
{
    let $errorText :=  "You reported on stocks waiting to be reclaimed in 4Aa. It is not plausible that these stocks are higher than your total beginning-of-year stocks in 4A. for gas [gas]. Please revise your data."

    for $gas in $report/F1_S1_4_ProdImpExp/Gas
    let $gasId := $gas/GasCode
    let $gasCode := $report/ReportedGases[GasId eq $gasId]/Code

    let $amount04A  := cutil:numberIfEmpty($gas/tr_04A/Amount, 0)
    let $amount04Aa := cutil:numberIfEmpty($gas/tr_04Aa/Amount, 0)

    where ($amount04Aa > $amount04A)

    return uiutil:buildRuleResult("2107", "4Aa", replace($errorText, "\[gas\]", $gasCode), $xmlconv:BLOCKER, true(), (), "")
    
};

declare function xmlconv:qc2108($report as element())
as element(div)*
{
    let $errorText :=  "You reported on stocks waiting to be reclaimed in 4Fa. It is not plausible that these stocks are higher than your total end-of-year stocks in 4F for gas [gas]. Please revise your data."

    for $gas in $report/F1_S1_4_ProdImpExp/Gas
    let $gasId := $gas/GasCode
    let $gasCode := $report/ReportedGases[GasId eq $gasId]/Code

    let $amount04F  := cutil:numberIfEmpty($gas/tr_04F/Amount, 0)
    let $amount04Fa := cutil:numberIfEmpty($gas/tr_04Fa/Amount, 0)

    where ($amount04Fa > $amount04F)

    return uiutil:buildRuleResult("2108", "4Fa", replace($errorText, "\[gas\]", $gasCode), $xmlconv:BLOCKER, true(), (), "")
    
};

declare function xmlconv:qc2109($report as element())
as element(div)*
{
    let $errorText :=  'You selected "EU Reclamation company" in the Activity section. Please report on quantities for reclamation or unselect the activity.'

    let $reclamCompIsSelected := cutil:is-activity-selected($report/GeneralReportData/Activities/RC)
    let $sum_amount04Aa := fgases:sum-of-gas-for-transaction($report/F1_S1_4_ProdImpExp, "tr_04Aa")
    let $sum_amount04Fa := fgases:sum-of-gas-for-transaction($report/F1_S1_4_ProdImpExp, "tr_04Fa")
    let $sum_amount04K := fgases:sum-of-gas-for-transaction($report/F1_S1_4_ProdImpExp, "tr_04K")

    return
         if($sum_amount04Aa > 0 or $sum_amount04Fa > 0 or $sum_amount04K > 0) then
            ()
        else
            if($reclamCompIsSelected) then
                uiutil:buildRuleResult("2109", "4Aa", $errorText, $xmlconv:BLOCKER, true(), (), "")
            else
                ()
        ...
        if(not($isExporter)) then
            ...
            return
                if(cutil:isEmptyString($companyId)) then
                    ...
                    for $entry in $doc/F2_S5_exempted_HFCs/Gas/tr_05C/TradePartner[TradePartnerID = $companyId]
                    
                    let $gasCode := $entry/../../GasCode
                    
                    let $gasAmount05C := number(($entry/amount)[1])

                    let $comment := ($entry/Comment)[1]
...
as element(div)*
{
    ...
    let $isDestructionCompany := cutil:is-activity-selected($report/GeneralReportData/Activities/D)
    ...
    for $gas in $report/F1_S1_4_ProdImpExp/Gas
        ...
        where
            ($tr01DAmount > ($tr01AbAmount + $tr04CAmount))
            or
            ($isDestructionCompany and $tr01DAmount > ($tr01AbAmount + $tr08EAmount))
    ...
    if(not(fgases:is-Eq-I-RACHP-HFC($report))) then
        ...
        return                
            if ($val13D > $val13A) then
                let $err_text := "Authorisations available in the HFC registry (13A) do not cover the demand for authorisations (13D; determined from your data in 11_A-11_F, 11_J1 and sections 12 and 12a). Please check your data in 11_A-11_F, 11_J1 and/or sections 12 and 12a to avoid errors. 
                Note that the European Commission (DG CLIMA) will assess your company’s quota/authorisation compliance in co-operation with your Member State’s competent authorities. Failure to comply may result in penalties according to national law of the Member State concerned"
...
as element(div)
{
    ...
    let $rStatusCopy := xmlconv:rule_CopyStatus($doc)
    ...
    let $resultDiv :=
        ...
        then
            ...
            let $r2107 := xmlconv:qc2107($doc)
            let $r2108 := xmlconv:qc2108($doc)
            let $r2109 := xmlconv:qc2109($doc)
            ...
            let $r2301_11A1a1 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1a1", 0.1, 4, "kg/piece", "2300")
            let $r2301_11A1a2 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1a2", 0.1, 4, "kg/piece", "2300")
            let $r2301_11A1a3 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1a3", 0.1,  4, "kg/piece", "2300")
            let $r2301_11A1a4 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1a4", 0.1,  4, "kg/piece", "2300")
            
            let $r2301_11A1b1 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1b1", 1, 15, "kg/piece", "2300")
            let $r2301_11A1b2 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1b2", 1, 15, "kg/piece", "2300")
            let $r2301_11A1b3 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1b3", 1, 15, "kg/piece", "2300")
            let $r2301_11A1b4 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1b4", 1, 15, "kg/piece", "2300")
            
            let $r2301_11A1c1 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1c1", 7, 500.0, "kg/piece", "2300")
            let $r2301_11A1c2 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1c2", 7, 500.0, "kg/piece", "2300")
            let $r2301_11A1c3 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1c3", 7, 500.0, "kg/piece", "2300")
            let $r2301_11A1c4 := xmlconv:validateTransactionAmountRange($doc, "tr_11A1c4", 7, 500.0, "kg/piece", "2300")
            
            let $r2301_11A2a1i := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a1i", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a1ii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a1ii", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a1iii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a1iii", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a2i := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a2i", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a2ii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a2ii", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a2iii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a2iii", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a3i := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a3i", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a3ii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a3ii", 0.2, 6, "kg/piece", "2300")
            let $r2301_11A2a3iii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2a3iii", 0.2, 6, "kg/piece", "2300")
            
            let $r2301_11A2b1i := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b1i", 1, 100.0, "kg/piece", "2300")
            let $r2301_11A2b1ii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b1ii", 1, 100.0, "kg/piece", "2300")
            let $r2301_11A2b1iii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b1iii", 1, 100.0, "kg/piece", "2300")
            let $r2301_11A2b2i := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b2i", 1, 500.0, "kg/piece", "2300")
            let $r2301_11A2b2ii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b2ii", 1, 500.0, "kg/piece", "2300")
            let $r2301_11A2b2iii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b2iii", 1, 500.0, "kg/piece", "2300")
            let $r2301_11A2b3i := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b3i", 1, 500.0, "kg/piece", "2300")
            let $r2301_11A2b3ii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b3ii", 1, 500.0, "kg/piece", "2300")
            let $r2301_11A2b3iii := xmlconv:validateTransactionAmountRange($doc, "tr_11A2b3iii", 1, 500.0, "kg/piece", "2300")
            
            let $r2301_11B1 := xmlconv:validateTransactionAmountRange($doc, "tr_11B1", 0.1, 1.0, "kg/piece", "2300")
            let $r2301_11B2 := xmlconv:validateTransactionAmountRange($doc, "tr_11B2", 0.1, 1000.0, "kg/piece", "2300")
            let $r2301_11B3 := xmlconv:validateTransactionAmountRange($doc, "tr_11B3", 0.1, 1000.0, "kg/piece", "2300")
            let $r2301_11B4 := xmlconv:validateTransactionAmountRange($doc, "tr_11B4", 0.1, 1000.0, "kg/piece", "2300")
            let $r2301_11B5 := xmlconv:validateTransactionAmountRange($doc, "tr_11B5", 0.1, 1000.0, "kg/piece", "2300")
            
            let $r2301_11C1a := xmlconv:validateTransactionAmountRange($doc, "tr_11C1a", 0.2, 6.0, "kg/piece", "2300")
            (: let $r2301_11C1b := xmlconv:validateTransactionAmountRange($doc, "tr_11C1b", 1.0, 3.0, "kg/piece", "2300") :)
            let $r2301_11C2a := xmlconv:validateTransactionAmountRange($doc, "tr_11C2a", 1.5, 1000.0, "kg/piece", "2300")
            (: let $r2301_11C2b := xmlconv:validateTransactionAmountRange($doc, "tr_11C2b", 3.0, 1000.0, "kg/piece", "2300") :)
            
            let $r2301_11D := xmlconv:validateTransactionAmountRange($doc, "tr_11D", 0.2, 1000.0, "kg/piece", "2300")
            
            let $r2301_11E1 := xmlconv:validateTransactionAmountRange($doc, "tr_11E1", 0.04, 13.0, "kg/piece", "2300")
            let $r2301_11E2 := xmlconv:validateTransactionAmountRange($doc, "tr_11E2", 0.025, 6.0, "kg/piece", "2300")
            let $r2301_11E3 := xmlconv:validateTransactionAmountRange($doc, "tr_11E3", 10.0, 50.0, "kg/piece", "2300")
            let $r2301_11E4 := xmlconv:validateTransactionAmountRange($doc, "tr_11E4", 0.02, 9.0, "kg/piece", "2300")
            
            let $r2301_11F1 := xmlconv:validateTransactionAmountRange($doc, "tr_11F1", 0.1, 1.5, "kg/piece", "2300")
            ...
            let $r2301_11F5 := xmlconv:validateTransactionAmountRange($doc, "tr_11F5", 0.2, 2.5, "kg/piece", "2300")
            ...
            let $r2301_11F8 := xmlconv:validateTransactionAmountRange($doc, "tr_11F8", 1.0, 20.0, "kg/piece", "2300")
            ...
            let $r2301_11J2 := xmlconv:validateTransactionAmountRange($doc, "tr_11J2", 0.007, 1.0, "kg/container", "2300")
            
            let $r2301_11K := xmlconv:validateTransactionAmountRange($doc, "tr_11K", 0.01, 0.5, "kg/container", "2300")
            ...
            let $r2301_11M1 := xmlconv:validateTransactionAmountRange($doc, "tr_11M1", 0.1, 5.0, "kg/piece", "2300")
            let $r2301_11M2 := xmlconv:validateTransactionAmountRange($doc, "tr_11M2", 2.0, 8.0, "kg/piece", "2300")
            let $r2301_11M3 := xmlconv:validateTransactionAmountRange($doc, "tr_11M3", 1.0, 105.0, "kg/piece", "2300")
            ...
            let $r2301_11N := xmlconv:validateTransactionAmountRange($doc, "tr_11N", 0.02, 50.0, "kg/piece", "2300")
