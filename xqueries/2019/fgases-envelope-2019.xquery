...
declare variable $SCHEMA as xs:string := "http://dd.eionet.europa.eu/schemas/fgases-2019/FGasesReporting.xsd";
declare variable $REPORT_TYPE as xs:string := "F-Gases";
declare variable $HELPDESK_EMAIL as xs:string := "bdr.helpdesk@eea.europa.eu";
...
as element(div)
{
    ...
    let $description :=
        if ($filesCountCorrectSchema = 0) then
            <span><span i18n:translate="">Your delivery cannot be accepted as you did not complete the online questionnaire as instructed in the BDR user manual.>URl:{$url} -- files: {$files} --filesCountCorrectSchema: {$filesCountCorrectSchema} </span>
<br/><br/><span i18n:translate="">URl:{$url} -- files: {$files} --filesCountCorrectSchema: {$filesCountCorrectSchema}  Please create a new envelope, fill in the online questionnaire and re-submit.</span></span>
