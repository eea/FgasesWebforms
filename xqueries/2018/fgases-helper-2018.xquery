...
declare variable $fgases:gas-list := doc('http://convertersbdr.eionet.europa.eu/xmlfile/fgases-gases.xml');
...
declare function fgases:get-gas-name-by-id($gasId as xs:string)
as xs:string
{
    if ($fgases:gas-list/FGases/Gas[GasId = $gasId] ) then
        $fgases:gas-list/FGases/Gas[GasId = $gasId]/Name
    else
        ""        
};
...
declare function fgases:get-gas-total-amounts($section, $transaction as xs:string)
as element(fgases:gasAmount)*
{
    for $transactionAmount in $section/Gas/*[local-name(.) = $transaction]
    let $gasId := string($transactionAmount/../GasCode)
    let $amount := string($transactionAmount/totalAmountForRow)
    let $comment := ""
    return fgases:_create-gas-amount($gasId, $amount, $comment)
};
...
declare function fgases:get-gas-total-amounts-with-values($section as element(), $transaction as xs:string)
as element(fgases:gasAmount)*
{
    for $gasAmount in fgases:get-gas-total-amounts($section, $transaction)
    let $value := fgases:get-amount-of-gas-amount($gasAmount)
    where not(empty($value)) and $value != 0
    return $gasAmount
};
...
as element(fgases:gasAmount)*
{
    ...
    for $item in $transaction
        ...
        let $amountObject :=
            for $tradepartner in $item/TradePartner
