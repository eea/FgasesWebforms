...
(:
 ...
 :
 : Version:     $Id: fgases-2018.xquery 22084 2018-03-10 13:31:33Z katsanas $
    ...
    return
        if ($err_flag) then
            ...
            let $actualTransactionYears :=
                for $actualYear in 2018 to fn:year-from-date(fn:current-date()) - 1
...
(: QC 24101 :)
    ...
    return
        if (not(fgases:is-P-HFC($doc)) and not(fgases:is-I-HFC($doc))) then
            ...
            where (($gasAmount13aA >= 0) and ($gasAmount13aB > $gasAmount13aA))
...
(: QC-2412 :)
    ...
    return
        if (count($invalidGases)>0) then
            uiutil:buildRuleResult("2412", "2A ", fn:concat("9F: ",$gasAmount09F), $xmlconv:WARNING, true(), $invalidGases , "Invalid gases are : ")
        else 
            ()
};
...
as element(div)*
{
    let $invalidGasTransactions :=
        for $gas in $gases        
        let $transactionData := $gas/tr_01A_fs
        where not(cutil:transactionHasMandatoryComment($transactionData))
        return
            let $gasName := cutil:getGasNameByGasCode($reportedData, $gas/GasCode)
    ...
    if (empty($report/F1_S1_4_ProdImpExp)) then
        ...
        return
            if ($isI or $isP) then
                ...
                let $amount := fgases:get-amount-of-gas-amount($gasAmount)
                return
                    if (empty($stock) and ((fn:not(fgases:has-comment-of-gas-amount($gasAmount)) and ($amount>0)) or ($amount=0))) then
                         
                                let $errorMsg := xmlconv:_compose-qc2055-error-message2($report, $transaction, $gasId, $xmlconv:BLOCKER)
                                return uiutil:buildRuleResult("2055", string($transaction/label), $errorMsg, $xmlconv:BLOCKER, true(), (), "")
                    else if (empty($stock)) then
                        ()
                     else  
                        ...
                        return
                            if ( cutil:getZeroIfNotNumber(string($amount)) < $stockAmount - 0.5) then
                                let $errorLevel :=
                                    if(
                                        fn:not(fgases:has-comment-of-gas-amount($gasAmount)) and ($amount>0)
                                    )
                                    then $xmlconv:BLOCKER else ($xmlconv:WARNING)
                            ...
                            else if ((fn:not(fgases:has-comment-of-gas-amount($gasAmount)) and ($amount>0)) or ($amount=0)) then
                                let $errorLevel :=
                                    if(
                                        fn:not(fgases:has-comment-of-gas-amount($gasAmount)) and ($amount>0)
                                    )
