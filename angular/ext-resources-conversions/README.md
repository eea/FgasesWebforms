...
There is a handful of company related data essential to each year's reporting, which are considered "registry data", however they do not derive from the BDR Fgases registry.
Instead, this data is extracted each year from the FDB and are as follows:
...
## Excel File
The excel file provided by EEA contains the following sheets. 

- 9A_imp (disaggregated_quota.py)
- NER List (ner.py)
- 9G (aggregated_quotas.py)
- 13A (authorisations.py)
- 4A,B,C,8E (stocks.py)
- large companies (large_companies.py)

Each of the above sheets has to be extracted to a csv file. Then each file has to be converted to xml by executing it's correspondent python script (In parenthesis above). The extracted xml files are actually input for the registry-data-merge-runner.xquery which has to be executed next. The result is the extra registry data xml file which has to be uploaded on bdr. For 2019 the link for the generated xml registry data file is https://bdr.eionet.europa.eu/fgases/xmlexports/fgases/fgases-extra-registry-data-2018.xml.

## Notes
The excel file contains UTF-8 and UTF-16 characters that are lost in the process of extracting a sheet to a csv file. For this reason 9A_imp sheet has to be saved as unicode text file first and then the tabs in the file hava to replaced with the apropriate delimiter (usually ";")