...
(function() {
    ...
    function($rootScope, $scope, $modalInstance, modalData, modalExtras, FormConstants, gasHelper, objectUtil, arrayUtil) {
                ...
                while(i--){
                    ...
                    if (typeof listOfComponets !== 'undefined' && !isEmpty(listOfComponets) && angular.isArray(listOfComponets)){
                        for (var k = 0; k < listOfComponets.length; k++){
                            if (component.ComponentId == listOfComponets[k].GasId){
        ...
        $scope.checkIfGasMixtureNameExists = function (createdGas) {
            ...
            if (angular.isDefined(modalExtras) && angular.isDefined(modalExtras.preDefinedMixtureList)){
                ...
                var matchFound = arrayUtil.selectSingle(preDefinedGasList, function (gas) {
                    ...
                    var createdGasName = createdGas.Code ? createdGas.Code.trim().toUpperCase() : null;
                    var comparedGasName = gas.Code ? gas.Code.trim().toUpperCase() : '';
                    var comparedGasNameVariant = comparedGasName.replaceAll('-', ''); //for the variant, delete all '-' characters
                    if (createdGasName === null) {
                        return false;
                    }
        ...
        $scope.ok = function(result){
            ...
            if (numberOfGases === 0 || (numberOfGases === 1 && !reportedGas.BlendComponents.Component[0].IsNonFGas)){ // if the selected gas is Non-fluor gas, accept only 1 gas to create a mixture
                $scope.alerts = [];
                var alert = {};
                alert.type = 'danger';
