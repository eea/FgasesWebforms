    ...
    angular.module('FGases.controllers').controller('CountrySpecificModalInstanceController', function($rootScope, $scope, $modalInstance, modalData, modalExtras) {
        ...
        $scope.ok = function(result) {
            ...
            if ($scope.NoCountrySpecified == true) {
                ...
                if (CountrySpec2ARow.CountryId == '00' && $scope.alerts.length == 0) {
                    var messageEUInterSelect = '';
                    if (modalExtras.trCountries == "tr_02A_Countries") {
                        messageEUInterSelect = 'Please note that \'EU intermediate storage\' is only eligible for HFCs released into the EU from a bonded warehouses under EU customs warehousing surveillance, where the HFCs had first been imported under the inward processing customs procedure and then (after inwards processing) been moved into a bonded warehouse. (The previous import for inward processing and the movement into the bonded warehouse may have been carried out by another undertaking.)';
                    } else if (modalExtras.trCountries == "tr_03A_Countries") {
                    ...
                    if (confirm(messageEUInterSelect)) {    //click OK

                    } else {    //click cancel
                        $scope.alerts = [];
                        var alert = {};
                        alert.type = 'danger';
                        alert.msg = 'Please confirm your choice or choose different country';
                        $scope.alerts.push(alert);
                    }
                }
                ...
                if ($scope.alerts.length == 0) {
                    ...
                    for (var i = 0; i < $scope.instance.FGasesReporting.F1_S1_4_ProdImpExp.Gas.length; i++) {
                        ...
                        if (($scope.containsHFC(gas)) || (modalExtras.trCountries == "tr_01A_fs_Countries")) {
                            console.log(modalExtras.trCountries);
                            ...
                            for (var c = 0; c < modalExtras.trRows.length; c++) {
                                if ($scope.instance.FGasesReporting.F1_S1_4_ProdImpExp.Gas[i][modalExtras.trRows[c]].CountrySpecific == null) {
                                    $scope.instance.FGasesReporting.F1_S1_4_ProdImpExp.Gas[i][modalExtras.trRows[c]].CountrySpecific = { "Country": null };
                                }
