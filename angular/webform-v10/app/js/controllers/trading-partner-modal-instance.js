...
(function() {
    ...
    angular.module('FGases.controllers').controller('TradingPartnerModalInstanceController', function($rootScope, $scope, $modalInstance, modalData, modalExtras, FormConstants, dataRepository, objectUtil, arrayUtil, stringUtil) {
        ...
        $scope.onlyRepresentativesNotRequired = modalExtras && modalExtras.onlyRepresentativesNotRequired;
        $scope.regisCodeNotRequired = modalExtras && modalExtras.regisCodeNotRequired;
        ...
        $scope.ok = function(result){
            ...
            if ($scope.tempPartnerDefinition.isEUBased == null){
                ...
                if ($scope.tempPartnerDefinition.EUVAT == null || $scope.tempPartnerDefinition.EUVAT.length == 0){
                    ...
                    (!$scope.isMinimalDialog && (!$scope.regisCodeNotRequired && ($scope.tempPartnerDefinition.NonEUDgClimaRegCode == null || $scope.tempPartnerDefinition.NonEUDgClimaRegCode.length == 0))) ||
                    (!$scope.isMinimalDialog && (!$scope.onlyRepresentativesNotRequired && ($scope.tempPartnerDefinition.NonEURepresentativeName == null || $scope.tempPartnerDefinition.NonEURepresentativeName.length == 0))) ||
                    (!$scope.isMinimalDialog && (!$scope.onlyRepresentativesNotRequired && ($scope.tempPartnerDefinition.NonEURepresentativeVAT == null || $scope.tempPartnerDefinition.NonEURepresentativeVAT.length == 0))) ) {
                    var alert = {};
                    alert.type = 'danger';
                    alert.msg = 'common.trading-partner-all-fields-should-be-set';
                    $scope.alerts.push(alert);
            ...
            if (!isEmpty(vat)) {
                ...
                if ($scope.fieldName === "tr_02H" && vat === $rootScope.vat) {
                    //QC 1068 for 2H
                    if ($scope.tempPartnerDefinition.NonEURepresentativeVAT === $rootScope.vat) {
                        // in this case, for H2 the VatNumber and OnlyRepresentative VatNumber could be the same
                    } else {
                        var alert = {};
                        alert.type = 'danger';
                                ...
                                if (modalExtras.partnerType == 'POM') {
                                    if (modalExtras.gasArray[i].tr_12A.Transaction[j].Exporter.TradePartnerID) {
                                        pair = {'POM': $scope.tempPartnerDefinition.CompanyName,
                                            ...
                                            'EXP': $scope.get_partner('Exporter', modalExtras.gasArray[i].tr_12A.Transaction[j].Exporter.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12A_TradePartners).partner.CompanyName,
                                    ...
                                    if (modalExtras.gasArray[i].tr_12A.Transaction[j].POM.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.get_partner('POM', modalExtras.gasArray[i].tr_12A.Transaction[j].POM.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12A_TradePartners).partner.CompanyName,
                ...
                if (angular.isDefined(modalExtras.yearField)) {
                    ...
                    var duplicate_tx = false;
                    var year_error_messages = {
                        'POM': 'sheet8.tr-12aA-pom-year-error',
                        'Exporter': 'sheet8.tr-12aA-exp-year-error'
                    };
                    var pair = [];
                    var pairs = $scope.get_transactions_pairs_12aA(modalExtras.txid);
                    // QC11201 & QC21212
                    for (var i = 0; i < modalExtras.gasArray.length; i++) {
                        for (var j = 0; j < modalExtras.gasArray[i].tr_12aA.Transaction.length; j++) {
                            if (modalExtras.gasArray[i].tr_12aA.Transaction[j].TransactionID == modalExtras.txid[modalExtras.gasArray[i].GasCode]) {
                                if (
                                    (modalExtras.partnerType == 'POM' && year > parseInt(modalExtras.gasArray[i].tr_12aA.Transaction[j].Exporter.Year)) ||
                                    (modalExtras.partnerType == 'Exporter' && year < parseInt(modalExtras.gasArray[i].tr_12aA.Transaction[j].POM.Year))
                                ) {
                                    yearOk = false;
                                    alert_msg = year_error_messages[modalExtras.partnerType];
                                }
                                pair = [];
                                if (modalExtras.partnerType == 'POM') {
                                    if (modalExtras.gasArray[i].tr_12aA.Transaction[j].Exporter.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.tempPartnerDefinition.CompanyName,
                                            'POMYear': parseInt($scope.tempPartnerDefinition.year),
                                            'EXP': $scope.get_partner('Exporter', modalExtras.gasArray[i].tr_12aA.Transaction[j].Exporter.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12aA_TradePartners).partner.CompanyName,
                                            'EXPYear': parseInt(modalExtras.gasArray[i].tr_12aA.Transaction[j].Exporter.Year)
                                        };
                                    }
                                } else {
                                    if (modalExtras.gasArray[i].tr_12aA.Transaction[j].POM.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.get_partner('POM', modalExtras.gasArray[i].tr_12aA.Transaction[j].POM.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12aA_TradePartners).partner.CompanyName,
                                            'POMYear': parseInt(modalExtras.gasArray[i].tr_12aA.Transaction[j].POM.Year),
                                            'EXP': $scope.tempPartnerDefinition.CompanyName,
                                            'EXPYear': parseInt($scope.tempPartnerDefinition.year)
                                        };
                                    }
                ...
                if (angular.isDefined(modalExtras.yearField)) {
                    ...
                    var pair = [];
                    var pairs = $scope.get_transactions_pairs_12B(modalExtras.txid);
                    // QC11201 & QC21212
                    for (var i = 0; i < modalExtras.gasArray.length; i++) {
                        for (var j = 0; j < modalExtras.gasArray[i].tr_12B.Transaction.length; j++) {
                            if (modalExtras.gasArray[i].tr_12B.Transaction[j].TransactionID == modalExtras.txid[modalExtras.gasArray[i].GasCode]) {
                                if (
                                    (modalExtras.partnerType == 'POM' && year > parseInt(modalExtras.gasArray[i].tr_12B.Transaction[j].Exporter.Year)) ||
                                    (modalExtras.partnerType == 'Exporter' && year < parseInt(modalExtras.gasArray[i].tr_12B.Transaction[j].POM.Year))
                                ) {
                                    yearOk = false;
                                    alert_msg = year_error_messages[modalExtras.partnerType];
                                }
                                pair = [];
                                if (modalExtras.partnerType == 'POM') {
                                    if (modalExtras.gasArray[i].tr_12B.Transaction[j].Exporter.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.tempPartnerDefinition.CompanyName,
                                            'POMYear': parseInt($scope.tempPartnerDefinition.year),
                                            'EXP': $scope.get_partner('Exporter', modalExtras.gasArray[i].tr_12B.Transaction[j].Exporter.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12B_TradePartners).partner.CompanyName,
                                            'EXPYear': parseInt(modalExtras.gasArray[i].tr_12B.Transaction[j].Exporter.Year)
                                        };
                                    }
                                } else {
                                    if (modalExtras.gasArray[i].tr_12B.Transaction[j].POM.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.get_partner('POM', modalExtras.gasArray[i].tr_12B.Transaction[j].POM.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12B_TradePartners).partner.CompanyName,
                                            'POMYear': parseInt(modalExtras.gasArray[i].tr_12B.Transaction[j].POM.Year),
                                            'EXP': $scope.tempPartnerDefinition.CompanyName,
                                            'EXPYear': parseInt($scope.tempPartnerDefinition.year)
                                        };
                                    }
                ...
                if (angular.isDefined(modalExtras.yearField)) {
                    ...
                    var pair = [];
                    var pairs = $scope.get_transactions_pairs_12aB(modalExtras.txid);
                    // QC11201 & QC21212
                    for (var i = 0; i < modalExtras.gasArray.length; i++) {
                        for (var j = 0; j < modalExtras.gasArray[i].tr_12aB.Transaction.length; j++) {
                            if (modalExtras.gasArray[i].tr_12aB.Transaction[j].TransactionID == modalExtras.txid[modalExtras.gasArray[i].GasCode]) {
                                if (
                                    (modalExtras.partnerType == 'POM' && year > parseInt(modalExtras.gasArray[i].tr_12aB.Transaction[j].Exporter.Year)) ||
                                    (modalExtras.partnerType == 'Exporter' && year < parseInt(modalExtras.gasArray[i].tr_12aB.Transaction[j].POM.Year))
                                ) {
                                    yearOk = false;
                                    alert_msg = year_error_messages[modalExtras.partnerType];
                                }
                                pair = [];
                                if (modalExtras.partnerType == 'POM') {
                                    if (modalExtras.gasArray[i].tr_12aB.Transaction[j].Exporter.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.tempPartnerDefinition.CompanyName,
                                            'POMYear': parseInt($scope.tempPartnerDefinition.year),
                                            'EXP': $scope.get_partner('Exporter', modalExtras.gasArray[i].tr_12aB.Transaction[j].Exporter.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12aB_TradePartners).partner.CompanyName,
                                            'EXPYear': parseInt(modalExtras.gasArray[i].tr_12aB.Transaction[j].Exporter.Year)
                                        };
                                    }
                                } else {
                                    if (modalExtras.gasArray[i].tr_12aB.Transaction[j].POM.TradePartnerID) {
                                        pair = {
                                            'POM': $scope.get_partner('POM', modalExtras.gasArray[i].tr_12aB.Transaction[j].POM.TradePartnerID, $scope.instance.FGasesReporting.F8_S12.tr_12aB_TradePartners).partner.CompanyName,
                                            'POMYear': parseInt(modalExtras.gasArray[i].tr_12aB.Transaction[j].POM.Year),
                                            'EXP': $scope.tempPartnerDefinition.CompanyName,
                                            'EXPYear': parseInt($scope.tempPartnerDefinition.year)
                                        };
                                    }
            ...
            if (!$scope.tempPartnerDefinition.isEUBased) {
                if ($scope.isMinimalDialog) {
                    ...
                    if ($scope.fieldName === "tr_02H" && $scope.tempPartnerDefinition.NonEUDgClimaRegCode == $rootScope.companyId) {
                        //QC 1069 for 2H
                        var alert = {};
                        alert.type = 'danger';
                ...
                if (companyData == null || companyData.exists == null || (companyData.exists == false && $scope.tempPartnerDefinition.isEUBased)) {
                    ...
                    dataRepository.checkCompanyExistByVatAndName($scope.tempPartnerDefinition.EUVAT, $scope.tempPartnerDefinition.CompanyName).error(function () {
                        ...
                        if (companyData == null || companyData.exists == null || companyData.exists == false) {
                            ...
                            var warned = false;
                            alert.msg = 'validation_messages.qc_' + qcCode + '.error_text';
                            for (var i=0; i<$scope.warnings.length; i++) {
                                if ($scope.warnings[i].msg === alert.msg) {
                                    warned = true;
                                }
                            }
                            if (!warned) {
                    ...
                    if ($scope.tempPartnerDefinition.NonEUDgClimaRegCode == null && $scope.regisCodeNotRequired) {
                        $scope.checkAndCloseIfSuccess();
                    } else {
                        //QC 1063 & 2074
                            ...
                            if (companyData == null || companyData.exists == null || companyData.exists == false) {
                                ...
                                dataRepository.checkCompanyExistByIdAndName($scope.tempPartnerDefinition.NonEUDgClimaRegCode, $scope.tempPartnerDefinition.CompanyName).error(function () {
                                    ...
                                    dataRepository.checkCompanyExistByIdAndCountryCodeOrVat($scope.tempPartnerDefinition.NonEUDgClimaRegCode, $scope.tempPartnerDefinition.NonEUCountryOfEstablishment['@id'], $scope.tempPartnerDefinition.NonEURepresentativeVAT).error(function () {
                                        ...
                                        if ((companyData == null || companyData.exists == null || companyData.exists == false) && !$scope.onlyRepresentativesNotRequired) {
                                            var alert = {};
                                            alert.type = 'danger';
