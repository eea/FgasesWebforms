...
(function() {
    ...
    function($rootScope, $scope, $modalInstance, modalData, modalExtras, FormConstants, gasHelper, objectUtil, arrayUtil) {
        ...
        //validate if mixture othes gas is one of the list
        $scope.validateMixtureOtherGases = function (element) {
            if (!objectUtil.isNull(element.currentTarget.value) && element.currentTarget.value.trim() != "") {
                var upperOtherGasList = $scope.otherODSGasesList.map(function (x) { return x.toUpperCase(); })
                var upperOtherGasListLess = $scope.otherODSGasesList.map(function (x) { return x.replace("-", "").toUpperCase(); })
                //if ($scope.otherODSGasesList.indexOf(element.currentTarget.value) == -1) {
                if ((upperOtherGasList.indexOf(element.currentTarget.value.toUpperCase()) >= 0) || (upperOtherGasListLess.indexOf(element.currentTarget.value.toUpperCase()) >= 0)) {
                    alert("Please note that you should not report on substances that deplete the ozone layer (ODS) in custom mixtures. In case such mixtures are traded/destroyed by your company, amounts reported in the F-gas questionnaire should be recalculated to exclude ODS components (if necessary on a best guess basis).");
                    element.currentTarget.value = "";
                }
            }
            
        };//end of function validateMixtureOtherGases

        //show alert message from entering custom gas blends without Annex I or II gases
        $scope.showQC1048Alert = function () {
            alert("Please note that none of the gases included in the custom mix are reportable gases according to Annex I or II or the F-Gas regulation. Custom blends without reportable gases cannot be created. Please refer to the FAQ document in the help section for further information.");
        };//end of function showQC1048Alert

        //validate if mixture gas name exists, QC_1049
        $scope.checkIfGasMixtureNameExists = function (createdGas) {
            var gasMixtureExist = false;
            if (angular.isDefined(modalExtras) && angular.isDefined(modalExtras.preDefinedMixtureList)){
                ...
                var matchFound = arrayUtil.selectSingle(preDefinedGasList, function (gas) {
                    ...
                    var createdGasName = createdGas.Code.trim().toUpperCase();
                    var comparedGasName = gas.Code.trim().toUpperCase();
                    var comparedGasNameVariant = comparedGasName.replaceAll('-', ''); //for the variant, delete all '-' characters
                    if (createdGasName === comparedGasName || createdGasName === comparedGasNameVariant)
                        return true;
                    else
                        return false;
                });
                if (matchFound != null)
                    gasMixtureExist = true;
            }
            return gasMixtureExist;
        };

        //show alert message when enters a custom gas mix with a name that exists on the Gases Shortlist
        $scope.showQC1049Alert = function () {
            alert("Please note that the mixture you are entering is provided as a predefined mixture in the lists provided under \"Selection of gases & mixtures\" above. Please select the corresponding entry in the drop-down menu.");
        };//end of function showQC1049Alert
        ...
        $scope.ok = function(result){
            ...
            var includingAnnexIorIIGas = false;
            ...
            for (var i = 0; i < numberOfGases; i++){
                ...
                //QC_1048: check if any gas is on the list of Annex I or II
                if (!component.IsOther && !component.IsNonFGas) {
                    includingAnnexIorIIGas = true;
                }
            }

            if (!includingAnnexIorIIGas) {
                $scope.showQC1048Alert();
                return; //user should at least choose one gas of Annex I or II list
            }

            //QC_1049: check if created mixture gas exists on the Gases Shortlist
            if ($scope.checkIfGasMixtureNameExists(reportedGas)) {
                $scope.showQC1049Alert();
                return;
            }
