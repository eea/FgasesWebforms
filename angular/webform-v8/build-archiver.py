...
PROD_SCHEMA_URL = 'http://dd.eionet.europa.eu/schemas/fgases-2023/FGasesReporting.xsd'


def createFormFileMetadata(filename, isTestDeployment, projectName):
	template = """{
        "title": "Fluorinated gases (F-Gases) (Article 19) (Transaction Year 2023)",
        "file": {
            "name": "%(filename)s"
        },
        "newXmlFileName": "fgases.xml",
        "emptyInstanceUrl": "https://%(webqHost)s/download/project/%(projectName)s/file/fgases-instance-empty-2023.xml",
...
def copyResources(isTestDeployment, projectName):
    ...
    if isTestDeployment:
        resources.extend(listTestResources())
            ...
            resources.extend([           
            '../xml/fgases-instance-empty-2023.xml'
        ])
...
def listTestResources():
    return [
        '../schema/FGasesReporting2023.xsd',
...
def copyEmptyInstanceForTest(projectName):
    with open('../xml/fgases-instance-empty-2023.xml') as source, open(BUILD_DIR + '/fgases-instance-empty-2023.xml', 'w') as dest:
        for line in source:
