...
PROD_SCHEMA_URL = 'http://dd.eionet.europa.eu/schemas/fgases-2025/Verification.xsd'
...
def createFormFileMetadata(filename, isTestDeployment, projectName):
	template = """{
        ...
        "emptyInstanceUrl": "https://%(webqHost)s/download/project/%(projectName)s/file/verification-instance-empty-2025.xml",
...
def copyResources(isTestDeployment, projectName):
    resources = [
            '../xml/fgases-labels-verification-2025-en.xml',
    ...
    if isTestDeployment:
        resources.extend(listTestResources())
            ...
            resources.extend([           
            '../xml/verification-instance-empty-2025.xml',
            '../xml/verification-instance-test.xml',
...
def copyEmptyInstanceForTest(projectName):
    with open('../xml/verification-instance-empty-2025.xml') as source, open(BUILD_DIR + '/verification-instance-empty-2025.xml', 'w') as dest:
        for line in source:
