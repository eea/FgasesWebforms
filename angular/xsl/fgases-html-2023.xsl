...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="labelsUrl">
		<xsl:choose>
			<xsl:when test="doc-available(concat($xmlPath, 'fgases-labels-2023-', $labelsLanguage ,'.xml'))">
				<xsl:value-of select="concat($xmlPath, 'fgases-labels-2023-', $labelsLanguage ,'.xml')"/>
			</xsl:when>
			<xsl:otherwise>
				...
				<xsl:if test="Activities/NIL-Report = 'true'">
					...
					<tr>
						<td style="padding-left-1-5em">
							<xsl:choose>
								<xsl:when test="Activities/NIL_sub100 = 'true'">
									<input class="floatRight" type="checkbox" name="nil-report-sub100" checked="true" disabled="true"></input>
								</xsl:when>
								<xsl:otherwise>
									<input class="floatRight" type="checkbox" name="nil-report-sub100" disabled="true"></input>
								</xsl:otherwise>
							</xsl:choose>
						</td>
						<th>
							<span>
								<xsl:call-template name="getLabel">
									<xsl:with-param name="labelName" select="'nil-report-sub100'"/>
								</xsl:call-template>
							</span>
							<br/>
							<span>
								<xsl:call-template name="getLabel">
									<xsl:with-param name="labelName" select="'nil-report-desc2'"/>
								</xsl:call-template>
							</span>
						</th>
					</tr>
							...
							<td style="text-align: right;">
								...
								<xsl:choose>
									<xsl:when test="../../../tr_12A_TradePartners/Partner[./PartnerId = current()/Exporter/TradePartnerID]/isEUBased=true()">
										...
										</div><span style="white-space: nowrap;">Year of placing on the EU Market: <xsl:value-of select="Exporter/Year"/></span>
                 ...
                 <tr> 
                        ...
                        <xsl:for-each select="$gases">
                          <xsl:variable name="isHfc"><xsl:value-of select="fgas:isHfcBased(current()/GasCode, /FGasesReporting)" /></xsl:variable>
						   <xsl:if test="$isHfc = true()">
                                <td  class="num_cell">
									<span><xsl:call-template name="formatValue"><xsl:with-param name="num" select="tr_02App/CountrySpecific/Country[CountryId = $countryId]/Amount/text()"/></xsl:call-template></span>
									<br/>
									<br/>
									<span class="comment"><xsl:call-template name="getValue"><xsl:with-param name="elem" select="tr_02App/CountrySpecific/Country[CountryId = $countryId]/Comment"/></xsl:call-template></span>
								</td>
                           </xsl:if>
                           <xsl:if test="$isHfc != true()">
                                 <td  class="num_cell">
                          ...
                          <xsl:variable name="isHfc"><xsl:value-of select="fgas:isHfcBased(current()/GasCode, /FGasesReporting)" /></xsl:variable>
						   <xsl:if test="$isHfc = true()">
                                <td  class="num_cell">
									...
									<br/>
									<br/>
									<span class="comment"><xsl:call-template name="getValue"><xsl:with-param name="elem" select="tr_02D/CountrySpecific/Country[CountryId = $countryId]/Comment/text()"/></xsl:call-template></span>
								</td>
                           </xsl:if>
                           <xsl:if test="$isHfc != true()">
                                 <td  class="num_cell">
			...
			<table class="tableSizeLimit table table-hover table-bordered">
				...
				<tbody>
					<tr>
						...
						<td class="total">
							<xsl:call-template name="formatValue"><xsl:with-param name="num" select="xs:string( sum(./SumOfAllHFCsS1/tr_10A[Amount != '']/Amount) )"/></xsl:call-template>
						</td>
						<td class="total">
							<xsl:call-template name="formatValue"><xsl:with-param name="num" select="xs:string( sum(./SumOfAllHFCsS2/tr_10A[Amount != '']/Amount) )"/></xsl:call-template>
						</td>
						<td class="total">
							...
							<xsl:if test="$isHfc = true()">
								<td class="textCenter total">
									<xsl:call-template name="getGas">
										<xsl:with-param name="elem" select="xs:string( sum(./tr_10A/TradePartner[amount != '']/amount) )"/>
									</xsl:call-template>
								</td>
							</xsl:if>
						</xsl:for-each>
					...
					<xsl:for-each select="tr_10A_TradePartners/*">
						...
						<tr>
							<td  class="padding-left-1em tradingPartners">
								...
								<xsl:if test="isEUBased = 'false'">
									<xsl:if test="string-length(NonEUDgClimaRegCode) > 0">
										<div class="padding-left-1em">
											Portal Code: <xsl:call-template name="getValue"><xsl:with-param name="elem" select="NonEUDgClimaRegCode"/></xsl:call-template>
										</div>
									</xsl:if>
									<div class="padding-left-1em">
										<xsl:call-template name="getLabel">
