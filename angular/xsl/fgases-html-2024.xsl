...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="labelsUrl">
		<xsl:choose>
			<xsl:when test="doc-available(concat($xmlPath, 'fgases-labels-2024-', $labelsLanguage ,'.xml'))">
				<xsl:value-of select="concat($xmlPath, 'fgases-labels-2024-', $labelsLanguage ,'.xml')"/>
			</xsl:when>
			<xsl:otherwise>
