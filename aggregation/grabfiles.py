...
FGASES_SCHEMA = 'http://dd.eionet.europa.eu/schemas/fgases-2018/FGasesReporting.xsd'
...
NIL_XSL = 'nil-sql.xsl'
REPORTING_YEAR = ['2018']
IGNORE_ENVELOPES = ['/10086/', '/fgas30000/', '/ods30000/', '/fgases/ro/colvnm0fa', '/ods/be/23',
      '/ods21128/envwqfbbg', 'ods20071/envwroy8q']
#IGNORE_ENVELOPES = []
SELECTED_ENVELOPES = []
...
def importCompanies():
    ...
    if obligation == FGASES_OBLIGATION and not SKIP_COMPANIES_HARVEST:
        registry_url = cdrserver + "european_registry/organisations_json?domain=FGAS"
        ...
        for org in response:
            if org['domain'] == 'FGAS':
                ...
                if testdata == 0 and org_id == '10086':
                    continue
                businessProfile = org.get('businessProfile', '')
...
def importRejected():
    ...
    if obligation == FGASES_OBLIGATION:
        ...
        for country in countryList:
            print >> sys.stderr, "grabbing rejected for: " + country           
            #unreleased_envelopes_api_url = cdrserver + "api/rejected_envelopes?obligation=" + obligation
            unreleased_envelopes_api_url = cdrserver + "api/envelopes?&fields=\"companyId\",\"url\",\"reportingDate\"&isBlockedByQCError=1&isReleased=1&obligation=" + obligation + "&countryCode=" + country
            req = urllib2.Request(unreleased_envelopes_api_url)
            base64string = base64.encodestring('%s:%s' % (cdruser, cdrpass)).replace('\n', '')
            req.add_header("Authorization", "Basic %s" % base64string)
            f = urllib2.urlopen(req)
...
#    args = [FGASES_OBLIGATION]
    countryList = ["AL","DZ","AD","AM","AT","AZ","BY","BE","BA","BG","HR","CY","CZ","DK","EG","EE","EU","FI","FR","GE","DE","GI","GR","GL","HU","IS","IE","IL","IT","JO","KZ","XK","KG","LV","LB","LY","LI","LT","LU","MK","MT","MD","MC","ME","MA","NL","NO","PS","PL","PT","RO","RU","SM","RS","SK","SI","ES","SE","CH","SY","TJ","TN","TR","TM","UA","GB","UZ","VA"]
    try:
