...
IGNORE_ENVELOPES = []
SELECTED_ENVELOPES = []
#SELECTED_ENVELOPES = ['ods90001/envwgvgtw', 'ods90002/envwh4hqa']
...
def harvestFiles():
    ...
    for envelope in r2['envelopes']:
        ...
        #work only with selected envelopes
        if not(envelope_selected(envelopeurl)):
            continue
...
if __name__ == '__main__':
    ...
    testdata = config.getboolean('cdr', 'testdata')
    if obligation == FGASES_OBLIGATION:
        tmpfilename = 'downloadtmp-fgases.xml'
    elif obligation == ODS_OBLIGATION:
        tmpfilename = 'downloadtmp-ods.xml'
